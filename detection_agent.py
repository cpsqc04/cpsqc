...
def sync_cameras_from_website(api_key: str) -> None:
    ...
    try:
        ...
        log("Synced cameras.json from website")
    except OSError:
        pass
...
def main() -> int:
    ...
    local_only = not api_key
    if local_only:
        log("No CCTV_FRAME_UPLOAD_KEY — local-only mode (detect.py + go2rtc; skip remote APIs).")
        scan_url = ""
        encoding_url = ""
        webrtc_url = ""

    log("Detection agent started (always-on monitoring).")
    log(f"  mode:   {'local-only' if local_only else 'remote-upload'}")
    ...
    last_cameras_sync = 0.0
    last_cameras_mtime = 0.0
    cameras_path = ROOT / "cameras.json"
    try:
        last_cameras_mtime = cameras_path.stat().st_mtime if cameras_path.is_file() else 0.0
    except OSError:
        last_cameras_mtime = 0.0

    while True:
        if api_key:
            # Pull Camera Management changes from Hostinger (stream type, IP, etc.).
            if (time.time() - last_cameras_sync) >= 8.0:
                try:
                    sync_cameras_from_website(api_key)
                except Exception as exc:  # noqa: BLE001
                    log_rate_limited("cam-sync", f"Camera config sync failed: {exc}", 60.0)
                last_cameras_sync = time.time()

        if scan_url and api_key:
            request_scan_poll_async(scan_url, api_key, min_interval=15.0)
        if encoding_url and api_key:
            request_encoding_poll_async(encoding_url, api_key, min_interval=8.0)

        # When cameras.json changes (local save or Hostinger sync), rebuild go2rtc for new stream type.
        try:
            mtime = cameras_path.stat().st_mtime if cameras_path.is_file() else 0.0
        except OSError:
            mtime = 0.0
        ...
        if webrtc_url and api_key and (time.time() - last_webrtc_publish) >= 20.0:
            publish_webrtc_status(webrtc_url, api_key)
            last_webrtc_publish = time.time()

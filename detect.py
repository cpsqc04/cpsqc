...
RECORDING_CHUNK_DURATION = 1800  # 30-minute recording segments (seconds)
MIN_RECORDING_DURATION = 60  # Discard very short fragments (< 1 min)
RECORDING_BUCKET_SECONDS = 1800  # Align filenames to 30-minute windows
...
# Disabled by default — monitoring + recording run 24/7 via the detection agent.
        ...
        if cameras_path.is_file():
            ...
            if isinstance(cameras, list):
                ...
                for cam in cameras:
                    ...
                    # Refresh RTSP path to match the stream the user selected.
...
def init_cctv_upload_from_env():
    ...
    try:
        chunk = int(
            env.get("CCTV_RECORDING_CHUNK_SECONDS", "")
            or os.environ.get("CCTV_RECORDING_CHUNK_SECONDS", "")
            or RECORDING_CHUNK_DURATION
        )
        if chunk >= 60:
            RECORDING_CHUNK_DURATION = chunk
            RECORDING_BUCKET_SECONDS = chunk
    except ValueError:
        pass
    try:
        days = int(
            env.get("CCTV_RECORDING_RETENTION_DAYS", "")
            or os.environ.get("CCTV_RECORDING_RETENTION_DAYS", "")
            or RECORDING_RETENTION_DAYS
        )
        if days >= 1:
    ...
    try:
        max_bytes = int(
            env.get("CCTV_RECORDING_MAX_STORAGE_BYTES", "")
            or os.environ.get("CCTV_RECORDING_MAX_STORAGE_BYTES", "")
            or RECORDING_MAX_STORAGE_BYTES
        )
        if max_bytes >= 50_000_000:
            RECORDING_MAX_STORAGE_BYTES = max_bytes
    except ValueError:
        pass
    try:
        min_dur = int(
            env.get("CCTV_RECORDING_MIN_SECONDS", "")
            or os.environ.get("CCTV_RECORDING_MIN_SECONDS", "")
            or MIN_RECORDING_DURATION
        )
        if min_dur >= 1:
            MIN_RECORDING_DURATION = min_dur
    except ValueError:
        pass
...
def get_camera_config_fingerprint():
    ...
    if not cfg:
        return None
    # Keys must match load_active_camera_config() return shape (snake_case).
    return (
        cfg.get("camera_id"),
        cfg.get("ipAddress"),
        cfg.get("port"),
...
def get_recording_bucket_start(dt=None):
    """Floor a timestamp to the current recording bucket (30-minute windows by default)."""
    dt = dt or datetime.now()
    bucket_span_min = max(1, int(RECORDING_BUCKET_SECONDS) // 60)
    bucket_minute = (dt.minute // bucket_span_min) * bucket_span_min
...
def cleanup_recordings_fifo(max_bytes=None):
    ...
    if max_bytes < 1 or not os.path.exists(RECORDINGS_DIR):
        return 0
    ...
    try:
        for filename in os.listdir(RECORDINGS_DIR):
            ...
            try:
                ...
                continue
            age = get_recording_file_age_seconds(filepath)
            # Sort key: older first (larger age). Missing age → use mtime via age helper fallback.
            sort_age = float(age) if age is not None else 0.0
            entries.append((sort_age, filepath, filename, size))
            total += size
    except OSError as e:
        print(f"✗ FIFO storage scan failed: {e}")
        return 0
    ...
    removed = 0
    for _age, filepath, filename, size in entries:
        if total <= max_bytes:
            break
        try:
            os.remove(filepath)
            ...
            total -= size
            removed += 1
            print(f"✓ FIFO storage cleanup removed oldest recording: {filename}")
        except OSError as e:
            print(f"✗ FIFO could not remove {filename}: {e}")

    if removed:
        gb = max_bytes / (1024 * 1024 * 1024)
        print(f"✓ FIFO cleanup complete ({removed} file(s) removed, cap {gb:.2f} GB)")
    return removed
...
def main():
    ...
    if ENABLE_RECORDING:
        try:
            ...
            print(f"✓ Storage cap: {RECORDING_MAX_STORAGE_BYTES / (1024 * 1024 * 1024):.2f} GB (FIFO delete oldest)")
            ...
            print("✓ Always-on: detection/recording continue even if Live Monitoring page is closed")
        except Exception as e:

...
/**
 * Admin endpoint: forward CCTV evidence to Incident Reporting.
 *
 * POST JSON:
 *   { "id": 1 }
...
try {
    ...
    if (!empty($request['forwarded_to_incident_reporting_at'])) {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'This CCTV request was already sent to Incident Reporting.',
            'data' => [
                'forwarded_to_incident_reporting_at' => $request['forwarded_to_incident_reporting_at'],
                'incident_reporting_evidence_reference_id' => $request['incident_reporting_evidence_reference_id'],
    ...
    $result = forwardCctvEvidenceToIncidentReporting($request);
    if (!$result['success']) {
    ...
    $noteLine = 'Sent to Incident Reporting for evidence'
    ...
    $update = $pdo->prepare('UPDATE cctv_requests SET
        status = :status,
        forwarded_to_incident_reporting_at = :forwarded_at,
        incident_reporting_evidence_reference_id = :reference_id,
    ...
    echo json_encode([
        ...
        'data' => [
            ...
            'forwarded_to_incident_reporting_at' => $timestamp,
            'incident_reporting_evidence_reference_id' => $referenceId,

...
def write_go2rtc_config(cameras: Optional[list] = None) -> Dict[str, Any]:
    ...
    sources: List[str] = []
    # Prefer Clear/main (Reolink app quality). ffmpeg→h264 helps browsers that reject HEVC WebRTC.
    if main_url:
    ...
    # YAML written manually to avoid adding PyYAML dependency.
    lines = [
...
def public_base_url() -> str:
    ...
    if configured:
        return configured
    # Default for on-site LAN browsers (HTTP Open Surveillance / same machine).
...
def write_status(running: bool, error: str = "", meta: Optional[dict] = None) -> dict:
    ...
    payload = {
        ...
        "stream": STREAM_NAME,
        "base_url": base,
        "player_url": f"{base}/stream.html?src={STREAM_NAME}&mode=webrtc,mse,hls",
        "webrtc_url": f"{base}/api/webrtc?src={STREAM_NAME}",
        "ws_url": f"{base.replace('https://', 'wss://').replace('http://', 'ws://')}/api/ws?src={STREAM_NAME}",
        "listen": (meta or {}).get("listen") or ":1984",
        "camera_id": (meta or {}).get("camera_id"),
        "camera_name": (meta or {}).get("camera_name"),
        "error": error or "",
